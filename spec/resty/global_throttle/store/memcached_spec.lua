...
describe("memcached", function()
  ...
  describe("incr and get", function()
    ...
    it("fails to add when expiry is decimal", function()
      local new_value, err = store:incr("client", 1, 1.5)
      assert.is_nil(new_value)
      assert.are.same("add failed for key 'client', expiry '1.5': CLIENT_ERROR bad command line format", err)
    end)

    it("ignores expiry when incrementing existing counter", function()
      incr_and_assert(store, "client5", 1, 1, 1)
      incr_and_assert(store, "client5", 1, 2, 1.5)
    end)
  end)
end)
